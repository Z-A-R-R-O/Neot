...
- [ ] Click block on canvas → selected in tree + properties panel populates
- [ ] Click block in tree → canvas selection glows
- [ ] Edit content in PropertiesPanel → canvas updates instantly (no reload)
- [ ] Double-click text on canvas → inline editor activates → type → blur saves
- [ ] Ctrl+Z undoes last edit → Ctrl+Shift+Z redoes
- [ ] Save named snapshot → restore it → blocks revert
- [ ] Switch to tablet/mobile → canvas resizes preview
- [ ] Delete block from tree → removed from canvas
- [ ] Publish → saves to DB → exits Dev Mode → shows "Published!" toast
